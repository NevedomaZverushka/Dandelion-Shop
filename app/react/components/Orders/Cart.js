...
function CartLink() {
  ...
  return (
    <>
      <div onClick={() => setModalWindow(true)} className="cart_link">
        <img src={cart} />
      </div>
...
function Order(props) {
  ...
  function handleCount(e, id) {
    ...
    bufOrder.products[
      order.products.findIndex((product) => product.product_data.id === id)
    ...
    return order.products.map(function (product) {
      return (
        <tr key={product.product_data.id}>
          <td>
            "{product.product_data.name}" - {product.product_data.author}
          </td>
          <td>
            <input
              onChange={(e) => handleCount(e, product.product_data.id)}
              type="number"
          ...
          <td>
            {Math.round(product.product_data.price * product.count * 0.2)}
          </td>
          <td>
            <button onClick={() => handleDeleteItem(product.product_data.id)}>
              Удалить
            </button>
          </td>
        </tr>
  ...
  function handleDeleteItem(id) {
    const bufOrder = order;
    bufOrder.products = order.products.filter(
      (product) => product.product_data.id !== id
    );
    setOrder(bufOrder);
    cookies.set("activeOrder", bufOrder);
    setItems(orderItems());
  ...
  if (order.status == 0) {
    return (
      <div id="order_wrap" className="order_wrap">
        <div className="order">
          <Helmet>
            <title>Заказ {String(order.order_id)}</title>
          </Helmet>

          <span onClick={props.closeWindow} className="close">
            &times;
          </span>
          ...
          {items.length == 0 ? (
            ...
            <>
              ...
              <center>
                ...
                {/* <button onClick={() => handleChangeStatus(order.order_id)}>
                  Подтвердить заказ
                </button> */}
              </center>
            </>
...
function Cart(props) {
  ...
  function handleActiveOrder(id) {
    axios.get("/change_active_order/" + id).then((response) => {
  ...
  const orders = ordersData.map(function (currOrder) {
    ...
    switch (currOrder.status) {
      case 0:
        ...
        break;
      case 1:
        orderStatus = "Ожидается возврат книги";
        break;
      case 2:
    ...
    return (
      <div key={currOrder.order_id} className="order_block">
        ...
        <table>
          ...
          <tbody>
            <tr>
              <td>
                <b>Дата заказа</b> -{" "}
                {timeFormatter.format(Date.parse(currOrder.created_at))}
              </td>
              <td rowSpan="3">
                <ol>
            ...
            <tr>
              <td>
                <b>Количество товаров</b> - {itemsCount}
              </td>
            </tr>
            <tr>
              <td>
                <b>Текущий статус заказа</b> - {orderStatus}
              </td>
            </tr>
            <tr>
              <td>
                <b>Общая сумма заказа</b> - {orderSum} грн.
              </td>
            </tr>
            <tr>
              <td>
                <b>Залог</b> - {Math.round(orderSum * 0.2)} грн.
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <center style={{ display: currOrder.status != 0 ? "none" : "" }}>
          <button onClick={() => handleActiveOrder(currOrder.order_id)}>
            Редактировать
          </button>
          <button onClick={() => handleDeleteItem(currOrder.order_id)}>
            Удалить
          </button>
          <button onClick={() => handleChangeStatus(currOrder.order_id)}>
            Подтвердить
          </button>
        </center>
      </div>
