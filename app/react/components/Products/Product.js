...
export default function Product(props) {
    ...
    return (
        <div className="product">
            <div className="img"
                style={{backgroundImage: props.data.image_url
                    ? 'url(' + props.data.image_url + ')'
                    : 'url(http://localhost:3000/packs/media/images/default_avatar-826a3503.jpg)'
                }}
            />
            <div>
                <h3>{props.data.name} - {props.data.author}</h3>
                <p className="description">{props.data.description}</p>
                <p className="price">Цена: {props.data.price} грн.</p>
            </div>
            <center style={{display: user != null ? '' : 'none'}}><button onClick={() => handleAddItem(props.data)}>Добавить в корзину</button></center>
            <div className="admin_product_block" style={{textAlign: 'center', display: (user == 1) ? '' : 'none'}}>
